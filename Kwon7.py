...
class BasicPaser(object):
    ...
    def _create_node(self, token):
        if token is None:
            ...
            return None
        elif token.type is CuteType.BOOLEAN_LIST:
            return Node(token.type)
        elif token.type is CuteType.BINARYOP_LIST:
            return Node(token.type)
...
def run_func(op_code_node):
    ...
    def cdr(node):
        ...
        """
        l_node = run_expr(node.value.next)
        l_node.value.next.value = l_node.value.next.value.next
        result = l_node.value.next
        return create_new_quote_list(result)
    ...
    def cons(node):
        ...
        print node.value.next.value
        print node.value.next.next.value
        print node.value.next.next.value.value
        print node.value.value
        print 'end------'

        if type(node.value.next.value) is Node:
            l_node = strip_quote(node.value.next)
            l_node.set_last_next(node.value.next.next.value.next.value)
            result = Node(TokenType.LIST, l_node)
            return create_new_quote_list(result)

        else:
            l_node = Node(TokenType.INT, node.value.next.value)
            l_node.set_last_next(node.value.next.next.value.next.value)
            return create_new_quote_list(l_node)

    def null_q(node):
        #Fill Out
        l_node = run_expr(node.value.next)
        if l_node.value.next.value is None:
            l_node.type = TokenType.TRUE
            return l_node
        else:
            l_node.type = TokenType.FALSE
            return l_node
    ...
    def atom_q(node):
        ...
        if l_node.value.next.value is None:
            l_node.type = TokenType.TRUE
            return l_node
        elif l_node.value.next.type is TokenType.LIST:
            l_node.type = TokenType.FALSE
            return l_node
        else:
            l_node.type = TokenType.TRUE
